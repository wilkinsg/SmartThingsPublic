...
metadata {
    ...
    tiles(scale: 2) {
        standardTile("contact", "device.contact", width: 6, height: 4) {
			state "open", label: '${name}', action: "switch.off", icon: "st.contact.contact.open", backgroundColor: "#00A0DC", nextState: "closed"
			state "closed", label: '${name}', action: "switch.on", icon: "st.contact.contact.closed", backgroundColor: "#cccccc", nextState: "open"
		}
